...
public class DragAndDrop_ : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (!hit.transform.GetComponent<PieceScript_>().in_right_position)
                {
                    selected_piece = hit.transform.gameObject;
                    selected_piece.GetComponent<PieceScript_>().selected = true;
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                selected_piece.GetComponent<PieceScript_>().selected = false;
                selected_piece = null;
            }
