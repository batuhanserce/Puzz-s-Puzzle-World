...
public class DragAndDrop_8x8_ : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            {
                if (!hit.transform.GetComponent<PieceScript_8x8_>().in_right_position)
                {
                    selected_piece = hit.transform.gameObject;
                    selected_piece.GetComponent<PieceScript_8x8_>().selected = true;
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            {
                selected_piece.GetComponent<PieceScript_8x8_>().selected = false;
                selected_piece = null;
            }
